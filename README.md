...
**DracFun Reborn 2.0.4**

Final release artifact:

`SFL_DracFun-Reborn2.0.4.jar`
...
DracFun Reborn 2.0.4 includes the completed clean-room restoration of the audited 134-ID legacy surface, Draconium progression and End resource generation, Energy Infuser, Item Converter, Fusion Crafting, modular equipment/modules, Energy Core multiblocks, Draconic Reactor, and Chaos Guardian encounter.
...
2.0.4 also restores the original Undying module's death-event revival contract, including its zero-XP revive path, tier revive health/invincibility values, and the legacy quirk where an eligible death starts the Undying cooldown before stored charge is checked.

Fresh 2.0.4 configurations enable the complete restoration by default. Every major system remains independently toggleable under `features:`, and existing explicit server configuration values are respected.

2.0.4 also detects the recognizable pre-completion development config that shipped only Materials enabled and safely migrates those old staged defaults to the completed feature set. Newer/manual feature choices are not overwritten.
...
Where practical, 2.0.4 defaults to the observable DracFun 2.0.10 behavior—even when the old addon had quirks. For example, the original Arrow Penetration integer-division bug is preserved by default. Server owners can opt into the corrected pierce behavior with:
